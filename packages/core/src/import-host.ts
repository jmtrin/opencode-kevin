...
export function parseClaudeMemory(dataRoot: string, env?: KevinEnv): { candidates: { content: string; type: string }[]; files_scanned: number; skipped_files: number; truncated: boolean } {
	...
	let files_scanned = 0;
	let skipped_files = 0;
	if (!existsSync(scanRoot)) return { candidates, files_scanned, skipped_files, truncated: false };
	let projectDirs: string[] = [];
	try { projectDirs = readdirSync(scanRoot); } catch { return { candidates, files_scanned, skipped_files, truncated: false }; }
	for (const proj of projectDirs) {
		...
		for (const f of files) {
			...
			if (content === null) { skipped_files++; continue; }
			// M-04: frontmatter without closing --- should skip
			if (content.trimStart().startsWith("---")) {
				const fmLines = content.replace(/\r\n/g, "\n").split("\n");
				let hasClosing = false;
				for (let i = 1; i < fmLines.length; i++) {
					if (fmLines[i].trim() === "---") { hasClosing = true; break; }
				}
				if (!hasClosing) { skipped_files++; continue; }
			}
...
export function parseCodexMemories(dataRoot: string, env?: KevinEnv): { candidates: { content: string; type: string }[]; files_scanned: number; skipped_files: number; truncated: boolean } {
	...
	if (!existsSync(scanRoot)) return { candidates, files_scanned, skipped_files, truncated: false };
	let files: string[] = [];
	try { files = readdirSync(scanRoot); } catch { return { candidates, files_scanned, skipped_files, truncated: false }; }
...
export function importHostMemories(opts: {
	...
	let parsed: { candidates: { content: string; type: string }[]; files_scanned: number; skipped_files: number; truncated: boolean };
	...
	const existingRows = opts.store.prepare("SELECT fingerprint FROM memories WHERE fingerprint IS NOT NULL").all() as { fingerprint: string }[];
